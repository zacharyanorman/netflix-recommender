...
# Collect titles from each matched genre
all_titles = []
for genre_id in matching_genres:
    ...
    if "results" in data:
        ...
        print(f"Results for genre {genre_id}: {genre_titles}")
        all_titles.extend(genre_titles)

# Match with IMDb data
...
# Get ratings and separate by type
rated_titles = []
for t in all_titles:
    r = get_rating(t)
    if r:
...
# Sort and separate
movies = sorted([r for r in rated_titles if r["type"] == "movie"], key=lambda x: x["rating"], reverse=True)[:5]
tv_shows = sorted([r for r in rated_titles if r["type"] in ["tvSeries", "tvMiniSeries"]], key=lambda x: x["rating"], reverse=True)[:5]
